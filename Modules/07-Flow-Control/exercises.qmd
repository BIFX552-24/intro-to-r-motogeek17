...
{
  print('There are records from 2012 in the gapminder dataset')
}
```
...
for (i in 1:5) {
  for (j in 1:5) {
    ...
    output_matrix[i, j] <- temp_output # by row-i stays the same for the first 5
  }
}
...
for (i in 1:5) {
  for (j in 1:5) {
    temp_j_value <- j_vector[j]
    temp_output <- paste(i, temp_j_value)
    output_matrix[j, i] <- temp_output # by column-j stays the same
    # instead of swapping indices, run t(output_matrix)
  }
}
output_vector2 <- as.vector(output_matrix)
```
...
for (i in levels(gapminder$continent)) {
 mean_lifeExp <- gapminder[gapminder$continent == i, 'lifeExp'] %>%
   unlist() %>% # takes a df/tibble and remove list structure, turning it into vector
    mean()
