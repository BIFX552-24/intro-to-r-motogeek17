...
```{r}
gap_africa <- filter(gapminder, continent == 'Africa') %>%
  select(lifeExp, country, year)
```

The subset of `gapminder` contains `r nrow(gap_africa)` rows. This corresponds to `r length(unique(gap_africa$country))` countries over `r length(unique(gap_africa$year))` years.
...
```{r}
avg_life_exp <- group_by(gapminder, country) %>%
  summarize(mean_life_Exp = mean(lifeExp)) %>%
  ungroup()
```

The country with the longest average life expectancy is `r arrange(avg_life_exp, desc(mean_life_Exp))$country[1]` and the country with the shortest average life expectancy is `r arrange(avg_life_exp, mean_life_Exp)$country[1]`.
